...
function NexusLib:CreateKeySystem(config)
    ...
    local note = config.Note or "Please enter your Premium Key to continue!"
    local keyLink = links.GetKey or config.KeyLink or "https://discord.gg/nexus"
    local keys = config.Keys or {"NEXUS-PREMIUM"}
    local callback = config.Callback or function() end
    ...
    local notifCon = Instance.new("Frame")
    notifCon.Name = "NotifContainer"
    notifCon.Size = UDim2.new(0, 320, 0, 400)
    notifCon.Position = UDim2.new(1, -340, 1, -420)
    notifCon.BackgroundTransparency = 1
    notifCon.Parent = sg
    local ncl = Instance.new("UIListLayout", notifCon)
    ncl.Padding = UDim.new(0, 10)
    ncl.VerticalAlignment = Enum.VerticalAlignment.Bottom
    ncl.SortOrder = Enum.SortOrder.LayoutOrder
    self._notifContainer = notifCon
    ...
    local lt = Instance.new("TextLabel")
    lt.Size = UDim2.new(0, 230, 0, 20)
    lt.Position = UDim2.new(0, 60, 0.5, -12)
    lt.BackgroundTransparency = 1
    lt.Text = title:upper()
    lt.TextColor3 = Theme.Accent or Color3.fromRGB(150, 80, 255)
    lt.TextSize = 16
    lt.Font = Theme.Bold
    lt.TextXAlignment = Enum.TextXAlignment.Left
    lt.Parent = topBar
    ...
    local lsub = Instance.new("TextLabel")
    lsub.Size = UDim2.new(0, 240, 0, 15)
    lsub.Position = UDim2.new(0, 60, 0.5, 8)
    lsub.BackgroundTransparency = 1
    lsub.Text = subtitle:upper()
    lsub.TextColor3 = Theme.TextMuted or Color3.fromRGB(140, 130, 170)
    lsub.TextSize = 10
    lsub.Font = Theme.Medium
    lsub.TextXAlignment = Enum.TextXAlignment.Left
    lsub.TextTruncate = Enum.TextTruncate.AtEnd
    lsub.Parent = topBar
    ...
    getBtn.MouseButton1Click:Connect(function()
        local linkToCopy = ""
        if type(config.GetKeyLink) == "function" then
            local link, err = config.GetKeyLink()
            if link then
                linkToCopy = link
            else
                NexusLib:Notify({Title = "Error", Message = err or "Could not get key link", Duration = 4})
                return
            end
        else
            linkToCopy = keyLink
        ...
        if setclipboard then
            ...
            NexusLib:Notify({Title = "Key System", Message = "Copied Get-Key link to Clipboard!", Duration = 4})
        else
            NexusLib:Notify({Title = "Error", Message = "Executor does not support setclipboard!", Duration = 4})
        end
    end)
    ...
    verifyBtn.MouseButton1Click:Connect(function()
        ...
        local valid = false
        local errStr = "Invalid or expired Key!"
        if type(config.VerifyKey) == "function" then
            valid, errStr = config.VerifyKey(entered)
            if errStr == nil then errStr = "Invalid or expired Key!" end
        else
        ...
        if valid then
            NexusLib:Notify({Title = "Success", Message = "Key verified successfully! Loading Hub...", Duration = 3})
            TweenService:Create(mainScale, TI.Close, {Scale = 0}):Play()
            ...
            NexusLib:Notify({Title = "Error", Message = errStr, Duration = 3})
    ...
    if hasShop then
        ...
        shopBtn.MouseButton1Click:Connect(function()
            if setclipboard and shop.Link then
                ...
                NexusLib:Notify({Title = "Shop", Message = "Copied Premium Shop link to Clipboard!", Duration = 4})
            end
        end)
    end
    ...
    task.defer(function()
        task.wait(1)
        NexusLib:Notify({Title = "Key System", Message = "Please enter your Key to continue!", Duration = 5})
    end)
end
...
function NexusLib:LaunchJunkie(junkieConfig)
    junkieConfig = junkieConfig or {}
    ...
    Junkie.service = junkieConfig.Service or "Free"
    Junkie.identifier = junkieConfig.Identifier or "1149704"
    Junkie.provider = junkieConfig.Provider or "Free"
    ...
    local function saveVerifiedKey(key)
        ...
        local ok = pcall(function()
            writefile("nexus_verified_key.txt", key)
        end)
        return ok
    end
    ...
    local function loadVerifiedKey()
        ...
        local ok, content = pcall(function()
            return readfile("nexus_verified_key.txt")
        end)
    ...
    if result and result.valid then
        ...
        -- Key is already valid, do not spawn UI
        return
    end

    local config = {
        Appearance = NexusLib.Appearance,
        Links = NexusLib.Links,
        Shop = NexusLib.Shop,
        Storage = NexusLib.Storage
    }
    
    config.GetKeyLink = function()
        return Junkie.get_key_link()
    end
    
    config.VerifyKey = function(key)
        ...
        return false, (res and res.error) or "Unknown Error"
    end
    ...
    NexusLib:CreateKeySystem(config)
    
    -- Yield execution until a valid Key is entered
    bindable.Event:Wait()
    bindable:Destroy()
end
