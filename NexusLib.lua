...
function NexusLib:Notify(config)
    ...
    local closeGlyph = label(close, "Ã—", 18, C.muted, Enum.Font.GothamMedium)
...
function NexusLib:CreateWindow(config)
    ...
    local hint = label(rail, "RIGHT ALT  Â·  TOGGLE", 9, C.muted, Enum.Font.GothamMedium)
    ...
    local minimize = createTopButton("MinimizeButton", "â€”", 14, C.hover, -52)
    local close = createTopButton("CloseButton", "Ã—", 16, C.danger, -12)
...
function Tab:CreateStatRow(stats)
    ...
    for i, stat in ipairs(stats) do
        ...
        corner(accent, 1)
        local value = label(core, tostring(stat.Value or "â€”"), 18, C.ivory, Enum.Font.GothamBold)
